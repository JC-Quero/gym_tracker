  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 font-sans relative">
      <main className="max-w-md mx-auto pb-32">
        ...
        {errorMsg && (
          <div className="bg-red-900/50 border border-red-500 text-white p-4 rounded-lg mb-4 text-center animate-pulse">
            <p className="font-bold">⏳ Esperando al Servidor...</p>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {!errorMsg && exercises.length === 0 && (
            <div className="text-center py-10 text-gray-500">
              Cargando ejercicios... <br/>
              (Si tarda mucho, Render se está despertando 😴)
            </div>
          )}
        </div>
          ...
          <div className="bg-gray-900 w-full max-w-md rounded-t-3xl p-6 border-t border-gray-700 animate-in slide-in-from-bottom duration-300 shadow-2xl">
            ...
            {history && history.found ? (
              <div onClick={copyHistory} className="bg-yellow-900/30 border border-yellow-700/50 p-3 rounded-lg mb-6 cursor-pointer hover:bg-yellow-900/50 transition-colors flex justify-between items-center group">
                ...
                <div className="text-yellow-500 text-xs font-bold bg-yellow-900/50 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">USAR</div>
              </div>
            ) : (
              <div className="mb-6 text-gray-500 text-xs italic text-center py-2">Primer registro de este ejercicio 🚀</div>
            )}
      ...
      {cart.length > 0 && !selectedExercise && (
        <div className="fixed bottom-6 left-0 right-0 px-6 z-40">
          <button onClick={finishWorkout} disabled={isSaving} className={`w-full max-w-md mx-auto font-bold py-4 rounded-2xl shadow-xl active:scale-95 transition-transform flex justify-center items-center gap-2 ${isSaving ? 'bg-gray-600 text-gray-300 cursor-not-allowed' : 'bg-green-500 hover:bg-green-400 text-black'}`}>
